 ...
 * 
 */
package com.zhilink.srm.common.security.shiro.session;
...
public class CacheSessionDAO extends EnterpriseCacheSessionDAO implements SessionDAO {
	 ...
	 * 不需要进行session更新的url
	 */
	private static String[] excluedUrls;

	public static String[] getExcluedUrls() {
		if (excluedUrls == null) {
			String excluedUrlsStr = Global.getConfig("web.session.exclude.url");
			if (StringUtils.isNotBlank(excluedUrlsStr)) {
				excluedUrls = excluedUrlsStr.split(",");
			}
		}
		return excluedUrls;
	}
    ...
    protected void doUpdate(Session session) {
		...
		if (request != null){
			...
			//特殊url不更新session,例如：定时获取信息的接口
			if (getExcluedUrls() != null && getExcluedUrls().length > 0) {
				for (String url : getExcluedUrls()) {
					if (StringUtils.endsWith(uri, url)) {
						return;
					}
				}
			}
