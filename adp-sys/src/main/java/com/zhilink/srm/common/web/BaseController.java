 ...
 * 
 */
package com.zhilink.srm.common.web;
...
public abstract class BaseController {
	...
	/**
	 * api接口参数检验
	 * @param resultModel api返回的实体
	 * @param object 校验对象
	 * @param groups 验证组
	 * @return
	 */
	protected boolean beanValidator(ResultModel resultModel, Object object, Class<?>... groups) {
		try {
			BeanValidators.validateWithException(validator, object, groups);
		} catch (ConstraintViolationException ex) {
			List<String> list = BeanValidators.extractPropertyAndMessageAsList(ex, ": ");
			list.add(0, "数据验证失败：");
			resultModel.setErrorMsg(list.toString());
			return false;
		}
		return true;
	}
