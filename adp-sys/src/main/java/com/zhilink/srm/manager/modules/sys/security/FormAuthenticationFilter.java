...
public class FormAuthenticationFilter extends org.apache.shiro.web.filter.authc.FormAuthenticationFilter {
	...
	// if (null != principal || isLoginRequest(request, response)) {// &&//
	// // isEnabled()
	// return Boolean.TRUE;
	// }
	// // API fix
	// // HttpServletRequest httprequest = (HttpServletRequest) request;
	// if (Global.isApiRequest(httprequest)) {
	// // api 的方式 直接抛出异常
	...
	public static void out(ServletResponse response, Map<String, Map<String, Object>> resultMap) {
	...
	protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
		if (isLoginRequest(request, response)) {
			...
			if (Global.isApiRequest(httprequest)) {
				...
				attributes.put("errorCode", Messages.ACCESS_EXCEPTION_CODE);
				attributes.put("errorMsg", I18n.i18nMessage("common.reLogin"));
