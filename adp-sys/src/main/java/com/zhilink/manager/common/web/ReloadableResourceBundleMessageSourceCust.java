...
/**
 * 
 * @ClassName: ReloadableResourceBundleMessageSourceCust
 * @Description: 国际化资源管理类,支持通配符设置资源文件，不再需要明确指定文件，可以支持不同的应用分别设置国际化资源
 * @author an48huf
 * @date 2017年11月1日
 *
 */
public class ReloadableResourceBundleMessageSourceCust extends AbstractMessageSource implements ResourceLoaderAware {
	...
	private long lastRefTimestamp = -1;
	...
	public Resource[] getResourcesFiles() {
		return resourcesFiles;
	}

	public void setResourcesFiles(Resource[] resourcesFiles) {	
	 ...
	 * <p>
	 * XML properties files are also supported: .g. "WEB-INF/messages" will find and
	 * load "WEB-INF/messages.xml", "WEB-INF/messages_en.xml", etc as well.
	 * 
	 * @param basename
	 *            the single basename
	 * @see #setBasenames
	 ...
	 * {@code java.util.ResourceBundle}. However, this is often not desirable in an
	 * application server environment, where the system Locale is not relevant to
	 * the application at all: Set this flag to "false" in such a scenario.
	 */
	 ...
	 * <li>A positive number will cache loaded properties files for the given number
	...
	protected String resolveCodeWithoutArguments(String code, Locale locale) {
		...
		if (this.cacheMillis < 0) {
			PropertiesHolder propHolder = getMergedProperties2(locale);
			...
			if (resourcesFiles != null && resourcesFiles.length > 0) {
				List<String> localeStrPart = createLocaleStrPart(locale);
				for (int k = resourcesFiles.length - 1; k >= 0; k--) {
					for (int j = localeStrPart.size() - 1; j >= 0; j--) {
						if (resourcesFiles[k].getFilename().indexOf(localeStrPart.get(j) + PROPERTIES_SUFFIX) >= 0) {
							PropertiesHolder propHolder = getProperties(resourcesFiles[k]);
							String result = propHolder.getProperty(code);
							if (result != null) {
								return result;
							}
	...
	protected void mergreLocaleResources() {
		List<Resource> resourcesLocale = new ArrayList<Resource>();
		for (int i = this.basenames.length - 1; i >= 0; i--) {
			try {
				Resource[] resources = resourcePatternResolver.getResources(basenames[i]);
				if (resources != null && resources.length > 0) {
					for (int k = 0; k < resources.length; k++) {
						resourcesLocale.add(resources[k]);
					}
				}
			} catch (IOException e) {
				e.printStackTrace();
	 ...
	 * @return
	 */
	protected PropertiesHolder getMergedProperties2(Locale locale) {
			...
			for (int k = resourcesFiles.length - 1; k >= 0; k--) {
				for (int j = localeStrPart.size() - 1; j >= 0; j--) {
					if (resourcesFiles[k].getFilename().indexOf(localeStrPart.get(j) + PROPERTIES_SUFFIX) >= 0) {
						...
						propHolder = refreshProperties(resourcesFiles[k], propHolder);
	 ...
	 * language code, country code, and variant code. E.g.: basename "messages",
	...
	protected List<String> createLocaleStrPart(Locale locale) {
	...
	protected PropertiesHolder getProperties(Resource resource) {
		PropertiesHolder propHolder = this.cachedProperties.get(resource.getFilename());
		long originalTimestamp = -2;
		...
		if (propHolder != null) {
			...
			propHolder = new PropertiesHolder();
			PropertiesHolder existingHolder = this.cachedProperties.putIfAbsent(resource.getFilename(), propHolder);
			if (existingHolder != null) {
				propHolder = existingHolder;
			}
		}
		...
		try {
			PropertiesHolder existingHolder = this.cachedProperties.get(resource.getFilename());
			if (existingHolder != null && existingHolder.getRefreshTimestamp() > originalTimestamp) {
				return existingHolder;
			}
			return refreshProperties(resource, propHolder);
		} finally {
			propHolder.refreshLock.unlock();
		}
	}
	...
	protected PropertiesHolder refreshProperties(Resource resource, PropertiesHolder propHolder) {
		...
		if (resource.exists()) {
			...
			if (this.cacheMillis >= 0) {
				...
				try {
					...
					if (propHolder != null && propHolder.getFileTimestamp() == fileTimestamp) {
						if (logger.isDebugEnabled()) {
							logger.debug("Re-caching properties for filename [" + resource.getFilename()
									+ "] - file hasn't been modified");
						}
						propHolder.setRefreshTimestamp(refreshTimestamp);
						return propHolder;
			...
			try {
				Properties props = loadProperties(resource, resource.getFilename());
				propHolder = new PropertiesHolder(props, fileTimestamp);
			} catch (IOException ex) {
				if (logger.isWarnEnabled()) {
					logger.warn("Could not parse properties file [" + resource.getFilename() + "]", ex);
		...
		propHolder.setRefreshTimestamp(refreshTimestamp);
		this.cachedProperties.put(resource.getFilename(), propHolder);
		return propHolder;
	}
