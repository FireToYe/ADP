...
package com.zhilink.sys.manager.entity;
 ...
 * 
 */
public class NewMenu extends DataEntity<NewMenu> {

	private static final long serialVersionUID = 1L;
	private NewMenu parent;	// 父级菜单
	...
	public NewMenu(String id){
		super(id);
	}
	
	@JsonBackReference
	@NotNull
	public NewMenu getParent() {
		return parent;
	}

	public void setParent(NewMenu parent) {
		this.parent = parent;
	}
	...
	@JsonIgnore
	public static void sortList(List<NewMenu> list, List<NewMenu> sourcelist, String parentId, boolean cascade){
		for (int i=0; i<sourcelist.size(); i++){
			NewMenu e = sourcelist.get(i);
			if (e.getParent()!=null && e.getParent().getId()!=null
				...
				if (cascade){
					...
					for (int j=0; j<sourcelist.size(); j++){
						NewMenu child = sourcelist.get(j);
