...
<ol id="${input}Preview"></ol><c:if test="${!readonly}"><a href="javascript:" onclick="${input}FinderOpen();" class="btn">${selectMultiple?fns:i18nMessage('common.add'):fns:i18nMessage('common.select')}</a>&nbsp;<a href="javascript:" onclick="${input}DelAll();" class="btn">${fns:i18nMessage('common.clear')}</a></c:if>
<script type="text/javascript">
	function ${input}FinderOpen(){//<c:if test="${type eq 'thumb'}"><c:set var="ctype" value="images"/></c:if><c:if test="${type ne 'thumb'}"><c:set var="ctype" value="${type}"/></c:if>
		...
		windowOpen(url,"${fns:i18nMessage('common.fileManagement')}",1000,700);
	...
	function ${input}Preview(){
		...
		if ($("#${input}Preview").text() == ""){
			$("#${input}Preview").html("<li style='list-style:none;padding-top:5px;'>${fns:i18nMessage('common.none')}</li>");
		}
	}
	${input}Preview();
</script>